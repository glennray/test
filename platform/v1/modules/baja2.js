var baja = new kendo.data.ObservableObject({
	...
	init: function() {
		...
		console.log("css version: " + config.css.version);
		...
		if ((getFreshCss || forceUpdate) && online) {
			console.log(config.resourcePath + config.css.url);
			$.ajaxSetup({ beforeSend: function (xhr) {  }, timeout:60000 });
			$.ajax({
			    ...
			    success: function(content) {
				    ...
				    console.log("css loaded remotely");
			   },
				complete: function() {
					$.ajaxSetup({ beforeSend: function (xhr) { xhr.setRequestHeader("Authorization", config.authString); }, timeout: moduleLoadTimeout*1000 });
				}
            ...
            if (localStorage.getObject("style.css") != null) {
                ...
                var stylesheet = $("<style></style>").append(content);
                $("head").append(stylesheet);
                console.log("css loaded from cache");
            }
            else {
                var css = $("<link rel='stylesheet' href='style.css'/>");
                $("head").append(css);
                console.log("css loaded from compiled version");
            }
		}
		...
		console.log("views version: " + config.views.version);
		...
		if ((getFreshViews || forceUpdate) && online) {
			console.log(config.resourcePath + config.views.url);
			$.ajaxSetup({ beforeSend: function (xhr) {  }, timeout:60000 });
			$.ajax({
				...
				success: function(content) {
					...
					console.log("views loaded remotely");
				},
				complete: function() {
					$.ajaxSetup({ beforeSend: function (xhr) { xhr.setRequestHeader("Authorization", config.authString); }, timeout: moduleLoadTimeout*1000 });
				}
            ...
            if (localStorage.getObject("views.html") != null) {
                ...
                $(appElement).append(content);
                console.log("views loaded from cache");
            }
            else {
                $.ajax({
                    url: "views.html",
                    ...
                    success: function(content) {
                        ...
                        localStorage.setObject("views.html", content);
                        localStorage.setObject("views.version", config.views.version);
                        console.log("views loaded from compiled version");
                    }
                });
            }
		}
	...
	initEvents: function() {
		...
		for(var i=0;i < config.events.length; i++) {
			$(document).live(config.events[i].type, function(event) {
				console.log("event fired: " + event.type);
				for(var j=0;j < config.events.length;j++) {
					if (event.type == config.events[j].type) {
						for(var k=0;k < config.events[j].items.length;k++) {
							if ($.trim(config.events[j].items[k]).length > 0) {
								eval($.trim(config.events[j].items[k]));
							}
						}
					}
				}
	...
	initHome: function() {
		...
		setTimeout(function() {
			...
			$("#home-inner").css("-webkit-transition", "opacity 2s").css("opacity", "1");
			debugTimeEnd = new Date();
			var diff = Date.DateDiff("ms", debugTimeStart, debugTimeEnd);
			console.log("** Elapsed time init: " + diff + " ms **");
		}, 200);
	...
	trackView: function(view) {
		if (useAnalytics) {
			if (kendo.support.mobileOS.name == "ios") {
				...
				window.plugins.analytics.trackPageView(view, function(){ console.log("TRACK VIEW: " + view); }, function(){ console.log("ERROR Tracking View"); });
			}
		}
	},
	...
	trackEvent: function(category, action, label, value) {
		if (useAnalytics) {
			if (kendo.support.mobileOS.name == "ios") {
				...
				window.plugins.analytics.trackEvent(category, action, label, 1, function(){ console.log("TRACK EVENT: " + action); }, function(){ console.log("ERROR Tracking Event"); });
			}
		}
	},
	...
	onTabSelect: function(tabIndex) {
		...
		//console.log("TAB SELECT VIEW: " + actualView);
	...
	shareFromMenu: function(method) {
		console.log("shareFromMenu: " + method);
		var message = "";
		var url = "";
		var subject = "";
		var body = "";
	...
	postTwitter: function(message, url) {
		...
		if (baja.get("os") == "ios") {
			...
			Twitter.isTwitterAvailable(
				...
				function(){
					console.log("We have a problem with the plugin");
				}
			);
		}
	},
...
console.log("baja module initialized");