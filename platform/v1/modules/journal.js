...
var journal = new kendo.data.ObservableObject({
	...
	dsJournals: new kendo.data.DataSource({
		...
		transport: {
			read: {
				url: function() { return config.journal.urlJournals.replace("{id}", journal.get("user") != null ? journal.get("user").Id : 0); },
				type: "GET",
				cache: true,
				dataType: "json",
		...
		change: function() {
			window.console && console.log("dsJournals.change fired");
	...
	dsEntries: new kendo.data.DataSource({
		...
		transport: {
			read: {
				url: function() { return config.journal.urlEntries.replace("{id}", journal.get("journal").get("Id")).replace("{p}", journal.get("page")).replace("{s}", journal.get("pageSize")); },
				type: "GET",
				dataType: "json",
				cache: false,
				contentType: "application/json"
		...
		schema: {
			...
			parse: function (data) {
				...
				for(var i=0;i < data.Entries.length;i++) {
					data.Entries[i].EntryDate = moment.utc(data.Entries[i].EntryDate).toDate();
				}
				return data;
			}
		},
	...
	init: function() {
		...
		$(document).live("user-login", function(event) {
			...
			window.console && console.log("user set");
		...
		window.console && console.log("journal module initialized");
	},

	installCustomJournals: function() {
		window.console && console.log("installing custom journals...");
		if (journal.get("user") != null && config.journal.custom != null && config.journal.custom.length > 0) {
			...
			for(var i=0;i < config.journal.custom.length;i++) {
				...
				window.console && console.log("looking for journal: " + def.id);
				var foundJournal = false;
				for(var j=0; j < journals.length;j++) {
					if (journals[j].CustomJournalId == def.id) {
						window.console && console.log("FOUND");
						foundJournal = true;
						break;
					}
				}
				if (!foundJournal) {
					window.console && console.log("NOT FOUND");
					var newJournal = {
						Id: 0,
						Title: def.title,
					...
					$.ajax({
						...
						success: function(savedJournal) {
							window.console && console.log("Journal " + savedJournal.Title + " created");
						},
	...
	editEntry: function(e) {
		...
		// custom journals
		window.console && console.log("entry CJI: " + entry.CustomJournalId);
		if (entry.CustomJournalId != null && config.journal.custom != null && config.journal.custom.length > 0) {
			window.console && console.log("setting up custom entry...");
			for(var i=0;i < config.journal.custom.length;i++) {
				var customDef = config.journal.custom[i];
				if (customDef.id == entry.CustomJournalId) {
					window.console && console.log("parsing custom content...");
					journal.set("customJournalDef", customDef);
					journal.set("currentPartIndex", 0);
	...
	saveEntry: function(e) {
		...
		var newEntry = {
			...
			EntryDate: moment.utc(kendo.parseDate($("#entry-edit-date").val()))
		};
		...
		if (edited.CustomJournalId != null) {
			...
			window.console && console.log("NEW CONTENT: " + html);
				...
				if (entryId == 0) {
					...
					for(var i=0;i < allEntries.length; i++) {
						if (allEntries[i].Id == entryId) {
                            window.console && console.log("updated allEntries");
							allEntries[i] = entry;
							break;
						}
					}
    				...
    				for(var i=0;i < entries.length; i++) {
						if (entries[i].Id == entryId) {
                            window.console && console.log("updated entries");
							entries[i] = entry;
							break;
						}
					}
	...
	cancelEditEntry: function(e) {
		if (baja.get("os") == "web") {
			if (window.confirm("If you leave this journal entry without saving you will lose any unsaved changes.\nAre you sure you want to do this?", "Just Checking...")) {
	...
	deleteEntry: function(e) {
		if (baja.get("os") == "web") {
			if (window.confirm("This will permanently delete your entry.\nAre you sure you want to do this?","Confirm Delete")) {
				var entryId = $("#entry_id").val();
	...
	openJournalEdit: function(e) {
		window.console && console.log("openJournalEdit");
	...
	initJournalEntries: function(e) {
		$("#list-journal-entries").kendoMobileListView({
			...
			click: function(e) {
                //window.console && console.log(e.dataItem);
		...
		if (item.CustomJournalId != null && config.journal.custom != null && config.journal.custom.length > 0) {
			for(var i=0;i < config.journal.custom.length;i++) {
				...
				if (customDef.id == item.CustomJournalId) {
					window.console && console.log("formatting custom content...");
					journal.set("customJournalDef", customDef);
    ...
    initJournalEntryEdit: function(e) {
    	window.console && console.log("initJournalEntryEdit");
        $("#entry-edit-title").live("focus", function(e) {
        	window.console && console.log("title focus");
		...
		if (!baja.get("isTablet")) {
			$("#entry-edit-div").bind("touchmove", function(e) {
				window.console && console.log("bind: editor touchmove");
				e.stopPropagation();
			});
		}
        ...
        $("#entry-edit-div").live("focus", function(e) {
        	...
        	window.console && console.log("editor focus");
        	if (baja.get("isTablet")) {
        ...
        $("#entry-edit-div").live("blur", function(e) {
        	window.console && console.log("editor blur");
        	if (baja.get("isTablet")) {
	...
	showJournalEntryEdit: function(e) {
		if (journal.get("refreshEditor")) {
			...
			if (!journal.get("scroller") && kendo.support.mobileOS.name == "ios") {
				if (!baja.get("isTablet")) {
					window.console && console.log("creating scroller");
			...
			window.console && console.log("edit stuff ready");
	...
	returnToView: function(e) {
		...
		window.console && console.log("RETURN TO: " + lastView);
		if (lastView != "journal") {
			e.preventDefault();
			window.console && console.log("redirecting to: " + lastView);
	...
	navigatePart: function(e) {
		...
		window.console && console.log(entry.parts[oldIndex].text);
